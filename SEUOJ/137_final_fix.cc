...
struct PlacedItem { int id; long long px, py, pz; long long sl, sw, sh; };

vector<Point3D> getValidRotations(const Item& item) {
    vector<Point3D> rotations;
    rotations.push_back({item.l, item.w, item.h});
    if (item.l != item.w) rotations.push_back({item.w, item.l, item.h});
...
bool isPointInsideAnyItem(const Point3D& pt, const vector<PlacedItem>& placedResults) {
    for (const auto& item : placedResults)
        if (pt.x >= item.px && pt.x < item.px + item.sl &&
            ...
            pt.z >= item.pz && pt.z < item.pz + item.sh) return true;
    return false;
}
void updateEPList(vector<Point3D>& epList, const PlacedItem& newItem,
                  const vector<PlacedItem>& placedResults) {
    epList.push_back({newItem.px + newItem.sl, newItem.py, newItem.pz});
    epList.push_back({newItem.px, newItem.py + newItem.sw, newItem.pz});
    epList.push_back({newItem.px, newItem.py, newItem.pz + newItem.sh});
    vector<Point3D> validEPs;
    for (const auto& ep : epList)
        if (!isPointInsideAnyItem(ep, placedResults)) validEPs.push_back(ep);
...
bool cmpLongestSide(const Item& a, const Item& b) {
    long long maxA = max({a.l, a.w, a.h}), maxB = max({b.l, b.w, b.h});
    return maxA > maxB;
}
...
int main() {
    ...
    mt19937 rng(42);
    vector<function<bool(const Item&, const Item&)>> strategies = { cmpVolume, cmpBaseArea, cmpHeight, cmpLongestSide };
    ...
    for (int i = 0; i < m; ++i) {
        ...
        for (auto& strategy : strategies) {
            ...
            if (packBox(boxes[i], temp, currentPlan)) { found = true; break; }
        }
        ...
        if (!found) {
            ...
            while (true) {
                ...
                if (packBox(boxes[i], shuffled, currentPlan)) { found = true; break; }
            }
        }
    ...
    if (bestBoxIdx == -1) { cout << "DORAYAKI\n"; return 0; }
    cout << boxes[bestBoxIdx].id << "\n";
    sort(bestPlan.begin(), bestPlan.end(), [](const PlacedItem& a, const PlacedItem& b) { return a.id < b.id; });

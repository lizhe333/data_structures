...
int main() {
    ...
    const double TIME_LIMIT = 18.0;  // 留 2 秒安全边际
    ...
    for (int i = 0; i < m; ++i) {
        if (totalItemVol > boxes[i].vol) continue;  // 体积预剪枝

        vector<Item> temp = items;
        sort(temp.begin(), temp.end(), cmpVolume);
        vector<PlacedItem> plan;

        if (packBox(boxes[i], temp, plan)) {
            bestBoxIdx = i;
            bestPlan = plan;
            break;  // 找到保底解，立刻停止 Phase 1
        }
    }
    ...
    for (int i = bestBoxIdx - 1; i >= 0; --i) {
        ...
        while (true) {
            ...
            if (packBox(boxes[i], shuffled, plan)) {
                bestBoxIdx = i;
                bestPlan = plan;
                improved = true;
                break;
